...
const LayerRow = memo(
  ({ layer, index }: { layer: DemoLayer; index: number }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs tabular-nums text-ink-muted/50 shrink-0 w-3.5">
          {index + 1}
        </span>
        <span
        ...
        />
        <span className="text-xs text-ink-muted/50 shrink-0 w-10">
          {layer.type}
        </span>
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {mode === 'raw' ? (
              ...
              >
                ...
                <p className="mt-2 text-xs text-ink-muted/40 text-center flex justify-center items-center gap-2">
